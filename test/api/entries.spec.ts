...
describe("Entries API test cases", () => {
  it("should check for entries is defined", async () => {
    const result = await makeEntries("blog_post").find<TEntry>();
    if (result.entries) {
  ...
  it("should set the include parameter to the given reference field UID", async () => {
    const query = await makeEntries("blog_post").includeReference("author").find<TEntry>();
    if (query.entries) {
  ...
  it("should check for include branch", async () => {
    const result = await makeEntries("blog_post").includeBranch().find<TEntry>();
    if (result.entries) {
  ...
  it("should check for include fallback", async () => {
    const result = await makeEntries("blog_post").includeFallback().find<TEntry>();
    if (result.entries) {
      expect(result.entries[0]._version).toBeDefined();
      expect(result.entries[0].locale).toEqual("en-us");
      expect(result.entries[0].uid).toBeDefined();
  ...
  it("should check for locale", async () => {
    const result = await makeEntries("blog_post").locale("fr-fr").find<TEntry>();
    if (result.entries) {
      expect(result.entries[0]._version).toBeDefined();
  ...
  it("should check for only", async () => {
    const result = await makeEntries("blog_post").locale("fr-fr").only("author").find<TEntry>();
    if (result.entries) {
